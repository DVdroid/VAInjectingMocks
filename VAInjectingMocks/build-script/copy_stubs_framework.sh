...
STUB_FRAMEWORK_PATH="${BUILT_PRODUCTS_DIR}/ToDosStub.framework"
...
copy_library() {
    ...
    rm -vRf  "${CODESIGNING_FOLDER_PATH}/PlugIns/ToDosStub.framework"
    mkdir -p "${CODESIGNING_FOLDER_PATH}/PlugIns/"
    cp -vRf "$STUB_FRAMEWORK_PATH" "${CODESIGNING_FOLDER_PATH}/PlugIns/ToDosStub.framework"
...
codesign_library() {
    ...
    if [ -n "${EXPANDED_CODE_SIGN_IDENTITY}" ]; then
        codesign -fs "${EXPANDED_CODE_SIGN_IDENTITY}" "${CODESIGNING_FOLDER_PATH}/PlugIns/ToDosStub.framework"
    fi
...
main() {
    ...
    for stub_configuration in $STUB_CONFIGURATIONS; do
        if [ "${CONFIGURATION}" = "$stub_configuration" ]; then
            ...
            echo "STUB_FRAMEWORK_PATH : $STUB_FRAMEWORK_PATH"
